    ...
    try:
        ...
        if not data:
            if allow_replay:
                raise HTTPException(
                    status_code=404,
                    detail="No recommendation available for today. Signal generation failed or no data available."
                )
            else:
                raise HTTPException(
                    status_code=404,
                    detail="No recommendation available for today. Signals are generated by the scheduled daily pipeline job at 12:00 UTC. Use allow_replay=true for manual generation (testing only)."
                )
